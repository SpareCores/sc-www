...
export class TrafficPricesComponent implements OnInit, OnDestroy {
  ...
  getTieredPriceText(price_tier: any) {
    let from =
      price_tier.lower && !isNaN(price_tier.lower)
        ? (price_tier.lower / 1000).toFixed(price_tier.lower < 1000 ? 2 : 0)
        : "0";
    let to =
      price_tier.upper &&
      !isNaN(price_tier.upper) &&
      price_tier.upper !== "Infinity"
  ...
  craftTitle(query: any) {
    ...
    let vendor =
      typeof query.vendor === "string"
        ? query.vendor
        : query.vendor?.length === 1
          ? query.vendor[0]
          : undefined;
    const direction =
      typeof query.direction === "string"
        ? query.direction
        : query.direction?.length === 1
          ? query.direction[0]
          : undefined;
