...
conference_talk_slides: "/assets/slides/hetzner-summit-2026.html#/coverslide"
...
::: {.notes}
Hi, I'm Gergely Daroczi from the Spare Cores team,
and we are an open-source project for cloud transparency,
so we try to compare cloud resources across vendors in a standardized, objective and transparent way.

Why do we do that?
:::


# {#no-cloud transition="convex-in slide-out" .center}

::: {.centered}
  <img src="images/no-cloud-just-another-comp.png" style="max-width: 75%; max-height: 50%;" />
:::

::: {.notes}
While pretty much everyone understands that cloud is still just another computer,
at a remote location, that I can spin up quickly and throw away as needed,
BUT we often don't really know what kind of computer that is.
:::

## {#old-vs-new transition="slide-in fade-out"}

::: {.notes}
Before the cloud, you could simply walk to the server, and
it was not too difficult to say that the computer running your program

- is a potato
- or a commodity PC with a case that has started turning yellow over the years
- or it's a bleeding edge, high-end server
- or not so new and it still has a turbo button

You could even open up the case when the system administrator was not watching ...
...
<div style="position: relative; display: inline-block; margin-top: -45px;">
  <img src="images/small-big-computer.jpg" style="display: block; width: 100%;">
</div>
:::


## {#vcpu transition="fade-in slide-out"}  

::: {.notes}
Like what on earth is even a virtual CPU?! Right?

There's a conceptual uncertainty and massive visibility problem here,
...
Which means that your applications could run much much faster!
:::
...
::: {.notes}
Now, I'd like to host a quick guessing game if you don't mind.

Any ideas how much faster is the best performing vCPU compared to the slowest?
I don't mean a comparing cloud servers with hundreds of cores with a potato,
indeed just focusing on the single-core performance -- like doing as many integer divisions in a second as possible.
:::

<div class="centered">
  <p style="font-size: 78px;">Guess the Number!</p>
  <p style="font-size: 56px; margin-top: 60px;">
...
::: {.notes}
Both the slowest and the fastest servers are from a hyperscaler .. not because I picked so,
it just happened to be the case.

Now I'm not going to give away the identity of the actual server types,
and shame them publicly ... especially in the era of awakening AGI (TODO autonomous general intelligence?).

But you can easily find out on your own if you are interested,
and I'm happy to show how.

But we do know both comes with 2 vCPUs.
...
<table style="font-size: 2rem; margin-left: auto; margin-right: auto;">
  ...
  <tbody>
    ...
    <tr class="fragment">
      <td style="background-color: #06263A;">Architecture</td>
      <td class="centered">x86_64</td>
      <td class="centered">x86_64</td>
    </tr>
    <tr class="fragment">
    ...
    <tr class="fragment">
      <td style="background-color: #06263A; color: #aaa;">Memory</td>
      <td class="centered" style="color: #aaa;">1 GiB</td>
      <td class="centered" style="color: #aaa;">8 GiB</td>
    </tr>
    <tr class="fragment">
...
::: {.notes}
Well, we measured! Not only these 2 servers, but actually close to 4,000 (3,800)!

So we solve this visibility problem of the cloud by burning hundreds of thousands of dollars to
...
<!-- 
https://sparecores.com/servers/compare?instances=W3siZGlzcGxheV9uYW1lIjoibThhem4ubGFyZ2UiLCJ2ZW5kb3IiOiJhd3MiLCJzZXJ2ZXIiOiJtOGF6bi5sYXJnZSIsInpvbmVzUmVnaW9ucyI6W119LHsiZGlzcGxheV9uYW1lIjoiZTItbWljcm8iLCJ2ZW5kb3IiOiJnY3AiLCJzZXJ2ZXIiOiJlMi1taWNybyIsInpvbmVzUmVnaW9ucyI6W119XQ%3D%3D
-->

![](images/vcpu-diff-1.png){.r-stretch fig-align="center"}

<div class="slide-text-overlay fragment"><span>30-50x</span></div>

::: {.notes}
logarithmic scale

note that this is a syntethic tets
...
<div class="slide-text-overlay fragment"><span>10-20x</span></div>

::: {.notes}
see Server A in the middle with scores around 500 across all dimensions?
so left hand side is single core, right hand side is multi core performance ... pretty much the same for Server A.

BUT Server B looking better: jumping from around 35 hundreds to 6000,
and in some workloads the diff between Server A and Server B are even larger.
...
<div class="slide-text-overlay fragment"><span>100x</span></div>

::: {.notes}
now this is not single-core anymore .. we run OpenSSL speed tests,
using various cipher and hash functions .. and Server A is hardly visible on this screen ..
...
<div class="slide-text-overlay fragment"><span>10-100x</span></div>
...
::: {.notes}
Back to Vincent from Pulp Fiction .. who now looks even more confused.

Because it's possible that you get a 32 vCPU server, and your app that perfectly scales to multiple cores,
runs slower than on a single core of another cloud server type.
:::
...
::: {.notes}
And there's quite a spectrum even when just looking at the 2 vCPU options ...
Any guesses how many?

[...]

611 instance types with 2 vCPUs ... we monitor 8 vendors (logos on prior slide).
...
.. well, swhen it comes to comparing Hetzner Cloud to other cloud providers,
just look around .. having the Summit right at a datacenter is pretty damn cool I must say :)

Haven't experienced that at other vendors, but I hope others might follow.

So now I might be a bit biased :)

But as said previously, we collect data in a transparent way and we don't have sponsored listings etc
Honestly the biggest value of Spare Cores is to remain independent and unbiased,
and I think you will also see that from some of this talk as well.

Anyway, I super excited not only for location, but when I received the invitation 
to give a talk here, I was working on my slides for a
...
<div class="slide-image-overlay fragment fade-in-then-out" data-fragment-index="1">
  <img src="images/Hetzner-Pressefotos-Falkenstein-DC-6.jpg" class="r-stretch">
</div>


## >>> from PyData import London {#pydata-london}

::: {.notes}
[...]

Basically pick a random Hetzner Cloud server for your smaller LLM serving needs.
:::

```sh {code-line-numbers="6-24|7-8|11-12|14-21|23-24|26-54|26-38,40-42,44-54|26-28,39,43,54|56-74|62|76-104|109-110" data-fragment-index="0" style="font-size:24px; margin-top: 20px !important; height: 640px;"}
...
<div class="slide-image-overlay fragment fade-in-then-out" data-fragment-index="5">
...
You were so great at guessing so far, so let me try again: any ideas what happened?

- no, no data issues. we are good :)

[...]

April 1st happened! quite a shock, honestly. all my beloved efficiency metrics potentially ruined in a second.

So you know how that changed the landscape? Are we still rocking the SLM (small language model) world?
...
::: {.notes}
Because before my PyData London talk, another price increase happened!
:::
...
and swallow the pill .. or other means of coping with the anxiety of the situation.

BTW if you wondered, I'm the guy in the back.
:::
...
::: {.notes}
And it's a good time to do a proper introduction, and why I'm interested in cloud transparency at all.

My background has been mostly in data engineer and data science, and managing infrastructure for such teams.

One example is a US startup spending around $5k/month on training GBM models, doing time-series forecasting etc.
And finding the most cost-efficient spot instances for these.
:::

## $ while [[ \$current != \$destination ]]

::: {.notes}
Which was fun! But I often found myself in the upper right quadrant of this chart,
which is not a good spot for a for-profit business entity who wants their data scientists and related cloud
folks to maximize profit .. and necessarily to find the most elegant solution.

So actually I went on a 6-months sabbatic in around 2022, and it worked out so well 
that I decided not coming back to work but stay in that mode, and actually called Spare Cores into life -- 
something I already wanted to do properly with a collegue and friend for a long time.
with Attila, who is actually sitting somewhere there.
:::
...
# $ whois sparecores.com {#whois transition="convex-in convex-out"}
...
<ul style="font-size: 0.9em;">
  ...
  <li class="fragment" data-fragment-index=3>Accepted into NVIDIA Inception, AWS Open Source Credits</li>
  <li class="fragment" data-fragment-index=4>Beneficiary of cloud credits from 8 vendors (overall ~$100k)</li>
  <li class="fragment" data-fragment-index=5>20+ talks in 9 countries (e.g. Berlin Buzzwords, PyData, KCD)</li>
...
# $ apropos sparecores.com {#apropos transition="convex-in convex-out"}
...
# $ curl -I sparecores.com/navigator {transition="convex-in slide-out"}

<img style="width:100%; margin-top: 30px;" src="images/homepage-stats-2026q3.png" />
...
## $ curl sparecores.com/search

::: {.centered}
<a href="https://sparecores.com/servers" target="_blank">
  <img style="width:95%;" src="images/sc-www-listing-top-2026q2.png" />
</a>
:::
...
## $ curl keeper.sparecores.net/docs
...
# $ ls -1 sparecores {#components transition="convex-in convex-out"}
...
# $ ls sc-images/llm-benchmark {transition="convex-in slide-out"}
...
<aside class="notes">
but when looking at a larger model, although the Azure server is still a bit faster for text generation than the AWS machine and can even do 512 tokens request (despite older CPU and much less memory), AWS is the clear winner for prompt processing
</aside>

## $ xdg-open vllm/status.png {.centered}

<img src="images/vllm-a10-45-chat-ttft-e2el-throughput.png" style="width: 100%;" class="centered"/>

<aside class="notes">
Also, it's not only througput and speed, but oftentimes you want to pick a server based on other requirements,
such as latency. Maybe you need to keep the Time to first token (TTFT) below a given treshold, or similar.

To help with that, we are taking the LLM benchmarks to the next level, by using vLLM and guideLLM 
to run multi-concurrent workloads and measure a bunch of related metrics -- stay tuned.
</aside>

## sqlite> SELECT * FROM scores ORDER BY price;

::: {.notes}
And just to highlight a few other workloads,
as although today everyone is talking about LLMs and AI,
we still have databases, websites and other important workloads .. 

So let me show one of our recent projects, that is evluating PostgreSQL performance on various servers.
TODO
:::

::: {.centered}
<img style="width:100%; margin-top: -30px;" src="images/www-server-search-db-hetzner.png" />

<p>Source: <a href="https://sparecores.com/servers?benchmark_score_min=1&memory_min=10&order_by=selected_benchmark_score_per_price&order_dir=desc&best_price_allocation=ONDEMAND_ONLY&columns=604473360&benchmark=eyJpZCI6InBnYmVuY2g6aGVhdnlfcmVhZF9vbmx5OnBlYWsiLCJjb25maWciOiJ7fSJ9" target="_blank">sparecores.com/servers</a></p>
:::

# $ cat benchmarks/NA.md {.centered}

::: {.notes}
Although we try hard, unforunately, we don't have a full coverage of all benchmarks across all vendors and instances.
:::

::: {.centered}
<img style="width:100%; margin-top: -30px; margin-bottom: -30px;" src="images/www-coverage-vendors.png" />

<p>Source: <a href="https://sparecores.com/navigator/benchmark-coverage" target="_blank">sparecores.com/navigator/benchmark-coverage</a></p>
:::
...
::: {.centered}
<img style="width:100%; margin-top: -30px; margin-bottom: -30px;" src="images/www-coverage-hetzner.png" />
:::


## $ cat benchmarks/NA.md

::: {.notes}
TODO ...


And the last disclaimer: (next)
:::
...
:::: {.centered}
<img src="https://media0.giphy.com/media/v1.Y2lkPTc5MGI3NjExZG5vdHcyaWJmeHVhcjZ2Zzl1dTNmZzgxZHd0YXM5Y2F5bDc3cHkyNyZlcD12MV9pbnRlcm5hbF9naWZfYnlfaWQmY3Q9Zw/N6mPyicEmayO93TMQt/giphy.gif" 
