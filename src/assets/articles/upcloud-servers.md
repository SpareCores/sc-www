...
1. **SKUs Listing**: We introduced UpCloud's API to our
   [Crawler](https://github.com/sparecores/sc-crawler) so that it knows how to
   query the list of predefined server types and their high-level specs from
   UpCloud. Using UpCloud's Python SDK for their API was a breeze.
...
3. **Pulumi Templates**: We created UpCloud-specific Pulumi templates in our
   [Runner](https://github.com/sparecores/sc-runner) component to implement
   server creation and deletion, including the related cloud resources. As
   UpCloud already had a Pulumi provider in Python, this was super
   straightforward as well. 
4. **Account Creation**: UpCloud generously provided us with plenty of cloud
   credits and extended our quotas to let us start all their server types for
   benchmarking — even in parallel, including their largest public node type
...
6. **Results**: The results are automatically published in our SQLite database
   files, public APIs, and webpage as well.
...
- **Cloud Native**: Highest compute performance and high (100%) SLA, flexible
  storage, network traffic included, but IPv4 comes at extra cost. Ideal for
...
All these server options come with the same CPU model (AMD EPYC 7542), and so
thus the presented SCore ("Spare Cores Score") is also very similar for all of
them. As a reminder, the SCore represent the number of `div16` ops using the
`stress-ng` CPU stress testing tool, which we use as a proxy for the raw CPU
performance. For more details, check out our [Performance
metrics](/article/cloud-compute-performance-benchmarks) blog post.

Looking at less synthetic benchmarks, we also see very similar performance, with
slight differences thanks to the different memory configurations. For example,
looking at the multi-core Geekbench benchmarks:
...
Once we look at higher scale models, the Developer plan is not available
anymore, but we have access to other CPU models. For example, looking at the 16
vCPU configurations: both the AMD EPYC 7542 and 7543 are available. Although the
newer generation CPU comes with the same CPU flags/features, it runs at a higher
base clock speed, and can reach higher boost frequencies as well, which
translates to higher performance, even just looking at the memory bandwidth
benchmarks:
...
Once we scale up to the largest nodes, we get access to AMD EPYC 9354 Zen 4 gen
CPU, which really boosts performance thanks to higher base clock speed, larger
CPU caches, and access to more powerful CPU instructions too, such as the
AVX-512. A good example might be compression workloads:
...
finding the right server type for your use case.

## Competitors

But how does UpCloud compare to other cloud providers? We have updated our
...
In general, when looking at the older generation Zen CPUs, the general SCore
values are not the strongest in the competition, but the Cloud Native plans
offer a very good performance/ratio, especially when looking at the PassMark
workloads, such as encryption or compression, or some of the memory read
operations.

It's also important to highlight that all UpCloud plans comes with dedicated CPU
cores, which means that the cores are not shared with other virtual machines,
it's not limited by boost credits, and the reported virtual processor cores
represent actual physical cores, not hyper-threaded logical cores, which is a
great advantage compared to the Intel-based cloud servers. As a quick example,
here's a comparison of UpCloud's 4-vCPU Cloud Native plan with four generations of
4-vCPU AWS servers with mixed Intel and ARM CPUs:
...
includes a generous outgoing (egress) traffic, which is not the case with most
other providers, and an IPv4 address is also included in the price, for which
all the other providers charge an extra ranging from 0.5 to 4 USD/month.

If you need custom configurations, UpCloud supports any combination of vCPUs and
memory amount with flexible storage, so you are not limited to the
above-mentioned predefined server types. This can be also a great benefit for
those who need to run specific workloads.
...
and we are looking forward to seeing more cloud providers joining us.
